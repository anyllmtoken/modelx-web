...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/${provider}/${canonicalId}`,
      languages: {
        zh: `/${provider}/${canonicalId}`,
        en: `/en/${provider}/${canonicalId}`,
        "x-default": `/${provider}/${canonicalId}`,
      },
    },
  ...
  const jsonLd = [
    {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 3,
          name: model.name,
          item: `${SITE_URL}/${provider}/${modelId}`,
        },
      ],
    },
    {
  ...
  return (
    <PageContainer>
      ...
      {familyModels.length > 1 && (
        <Section
          id="family"
          title={`${t("familyComparison")}: ${model.family}`}
        >
          <FamilyComparison
            models={familyModels}
            currentId={model.id}
      ...
      <Section id="api" title={t("api")}>
        <ApiEndpoint
          path={`/v1/models/${model.provider}/${model.id}`}
          tryPath={`https://api.anyllmtoken.com/v1/models/${model.provider}/${model.id}`}
        />
      </Section>
