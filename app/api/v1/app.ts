...
// ── Create Hono app (routes are path-relative, prefix stripped by route files) ──
...
app.use(
  ...
  rateLimiter({
    ...
    skip: (c) => c.req.path === "/" || c.req.method === "OPTIONS",
  }),
);
  ...
  return c.json(
    ok({
      ...
      endpoints: {
        ...
        pricing: "/api/v1/pricing/compare",
        mcp: "/api/mcp",
      },
    }),
  );
...
// ── GET /models/compare ── (must be before :provider/:id catch-all)
  ...
  if (format === "csv") {
    ...
    return new Response(csv, {
      headers: {
        ...
        "Content-Disposition": "attachment; filename=ModelX-models.csv",
      },
    });
  }
